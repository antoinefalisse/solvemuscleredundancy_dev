...
function [Time,MExcitation,MActivation,RActivation,TForcetilde,TForce,lMtilde,lM,MuscleNames,OptInfo,DatStore]=SolveMuscleRedundancy_FtildeState_actdyn_CasADi_parallel(model_path,IK_path,ID_path,time,OutPath,Misc)
...
% Declare variables for setting up parallel computing.
ak = MX.sym('ak',auxdata.NMuscles);
akmesh = MX.sym('akmesh',auxdata.NMuscles,d);
akj = [ak akmesh];

FTtildek = MX.sym('FTtildek',auxdata.NMuscles);
FTtildekmesh = MX.sym('FTtildekmesh',auxdata.NMuscles,d);
FTtildekj = [FTtildek FTtildekmesh];

vAk = MX.sym('vAk',auxdata.NMuscles);
aTk = MX.sym('aTk',auxdata.Ndof);
dFTtildek = MX.sym('dFTtildek',auxdata.NMuscles);

LMTinterpk = MX.sym('LMTinterpk',auxdata.NMuscles);
VMTinterpk = MX.sym('VMTinterpk',auxdata.NMuscles);
IDinterpk = MX.sym('IDinterpk',auxdata.Ndof);
MAinterpk = MX.sym('MAinterpk',auxdata.Ndof*auxdata.NMuscles);

% Defect constraints (evaluated in parallel across mesh points.)
% ===================================================================
J = 0;
constr = {};
...
for j=1:d
    ...
    ap = akj*C(:,j+1);
    FTtildep = FTtildekj*C(:,j+1);        
    % Append collocation equations
    % Activation dynamics (explicit formulation)
    constr{end+1} = h*vAk.*auxdata.scaling.vA - ap;
    % Contraction dynamics (implicit formulation)
    constr{end+1} = h*dFTtildek.*auxdata.scaling.dFTtilde - FTtildep;
    % Add contribution to the quadrature function
    J = J + ...
        B(j+1)*f_ssNMuscles(akj(:,j+1)')*h + ...
...
f_coll = Function('f_coll',{ak, akmesh, FTtildek, FTtildekmesh, vAk, aTk, ...
    dFTtildek}, {constr, J});

f_coll_map = f_coll.map(N, 'thread', 8);
[coll_constr, Jall] = f_coll_map(a(:,1:end-1), amesh, FTtilde(:,1:end-1), ...
    FTtildemesh, vA, aT, dFTtilde);
opti.subject_to(coll_constr==0);
J = sum(Jall);
opti.minimize(J);

% Path constraints (evaluated in parallel across mesh points.)
% =============================================================
LMTinterpTrans = LMTinterp'; 
VMTinterpTrans = VMTinterp';
IDinterpTrans = IDinterp';
MAinterpTrans = MAinterp';
constr = {};
constrAct1 = {};
constrAct2 = {};
% Get muscle-tendon forces and derive Hill-equilibrium
[Hilldiffk,FTk] = f_forceEquilibrium_FtildeState(ak', FTtildek', ...
    dFTtildek'.*auxdata.scaling.dFTtilde,LMTinterpk,VMTinterpk);
constr{end+1} = Hilldiffk;
% Moment constraints
for dof = 1:auxdata.Ndof
    T_exp = IDinterpk(dof);
    index_sel = (dof-1)*(auxdata.NMuscles)+1:(dof-1)*(auxdata.NMuscles)+auxdata.NMuscles;
    T_sim = f_spNMuscles(MAinterpk(index_sel),FTk) + auxdata.Topt*aTk(dof);
    constr{end+1} = T_exp-T_sim;
end    
% Activation dynamics constraints
act1 = vAk*auxdata.scaling.vA + ak/tdeact;
constrAct1{end+1} = act1;
act2 = vAk*auxdata.scaling.vA + ak/tact;
constrAct2{end+1} = act2;

constr = vertcat(constr{:});
constrAct1 = vertcat(constrAct1{:});
constrAct2 = vertcat(constrAct2{:});
f_path = Function('f_coll',{ak, FTtildek, dFTtildek, LMTinterpk, VMTinterpk, ...
    IDinterpk, MAinterpk, aTk, vAk}, {constr, constrAct1, constrAct2});
f_path_map = f_path.map(N, 'thread', 8);
[path_eq_constr, path_ineq_constr1, path_ineq_constr2] = ...
    f_path_map(a(:,1:end-1), FTtilde(:,1:end-1), dFTtilde, ...
    LMTinterpTrans(:,1:end-1), VMTinterpTrans(:,1:end-1), ...
    IDinterpTrans(:,1:end-1), MAinterpTrans(:,1:end-1), aT, vA);
opti.subject_to(path_eq_constr == 0);
opti.subject_to(path_ineq_constr1{:} >= 0);
opti.subject_to(path_ineq_constr2{:} <= 1/tact);

% State continuity at mesh transition (not in parallel).
% ======================================================
for k=1:N
