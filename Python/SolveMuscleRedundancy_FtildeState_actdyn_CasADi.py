...
def SolveMuscleRedundancy_FtildeState_actdyn_CasADi(model_path,IK_path,ID_path,time,OutPath,misc):
...
# Check for optional input arguments ------------------------------------ #
    ...
    # Inverse Dynamics
    if not 'f_cutoff_ID' in misc:
        misc['f_cutoff_ID'] = 6  
        
    if not 'f_order_ID' in misc:
        misc['f_order_ID'] = 6 
        
    # Muscle-tendon lengths
    if not 'f_cutoff_lMT' in misc:
        misc['f_cutoff_lMT'] = 6  
        
    if not 'f_order_lMT' in misc:
        misc['f_order_lMT'] = 6 
        
    # Moment arms
    if not 'f_cutoff_dM' in misc:
        misc['f_cutoff_dM'] = 6  
        
    if not 'f_order_dM' in misc:
        misc['f_order_dM'] = 6 
        
    # Inverse Kinematics
    if not 'f_cutoff_IK' in misc:
        misc['f_cutoff_IK'] = 6  
        
    if not 'f_order_IK' in misc:
        misc['f_order_IK'] = 6 
        
    # Mesh Frequency
    if not 'Mesh_Frequency' in misc:
...
#    if isempty(ID_path) || ~exist(ID_path,'file'):
    ...
    if not ID_path:
        np.disp('ID path was not specified or the file does not exist, computation ID started')
        
        if not 'Loads_path' in misc:
            raise Exception('No path provided for external loads')
        else:
            # Check the output path for the ID results
            if 'ID_ResultsPath' in misc:
                idpath = os.path.dirname(misc['ID_ResultsPath'])                
                if not os.path.isdir(idpath):
                    os.mkdir(idpath)
            else:
                # Save results in the directory of the external loads
                misc['ID_ResultsPath'] = misc['Loads_path']  
                
            (ID_outPath, fileName) = os.path.split(misc['ID_ResultsPath'])
            (fileBaseName, fileExtension) = os.path.splitext(fileName)
            
            output_settings=os.path.join(ID_outPath,fileBaseName + '_settings.xml')
            
            main_path = os.path.dirname(os.path.dirname(os.getcwd()))
            sys.path.append(os.path.join(main_path,'MuscleModel','ReadFromModel'))
            
            from Opensim_ID import Opensim_ID                  
            time_adj = np.append(time[0]-0.1,time[1]+0.1)            
            Opensim_ID(model_path,time_adj,misc['Loads_path'],IK_path,ID_outPath,fileName,output_settings)
            ID_path = os.path.join(ID_outPath,fileName)

            # ----------------------------------------------------------------------- #
            # Muscle analysis ------------------------------------------------------- #
            
            misc['time']=time
            MuscleAnalysisPath = os.path.join(OutPath,'MuscleAnalysis')
            print(MuscleAnalysisPath)
            if not os.path.isdir(MuscleAnalysisPath):
                os.makedirs(MuscleAnalysisPath)           
            np.disp('MuscleAnalysis Running .....')
            from OpenSim_Muscle_Analysis import OpenSim_Muscle_Analysis  
            time_MA = np.append(time[0],time[1])
            OpenSim_Muscle_Analysis(IK_path,model_path,MuscleAnalysisPath,time_MA)
            np.disp('MuscleAnalysis Finished')
...
#return Time,MExcitation,MActivation,RActivation,TForcetilde,TForce,lMtilde,lM,MuscleNames,OptInfo,DatStore
    return time[0]